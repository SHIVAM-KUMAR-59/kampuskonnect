...
const updateStudentProfileService = async (user, updateData) => {
    try {
        const { name, email, phoneNumber, bio, linkedinUrl, branch, interests, graduationYear } = updateData;
        ...
        if (graduationYear !== undefined) {
            ...
            if (year < currentYear || year > currentYear + 10) {
                throw new ApiError(`Graduation year must be between ${currentYear} and ${currentYear + 10}`);
            }
